...
# isgctwing 停止脚本
# 版本: v1.0.0
# 功能: 通过 isgservicemonitor 停止 isgctwing 服务
# =============================================================================
...
get_isgctwing_pid() {
    local port_pid=$(netstat -tnlp 2>/dev/null | grep ":$isgctwing_PORT " | awk '{print $7}' | cut -d'/' -f1 | head -n1)
    
    if [ -n "$port_pid" ] && [ "$port_pid" != "-" ]; then
        local process_name=$(ps -p "$port_pid" -o comm= 2>/dev/null)
        if [ -n "$process_name" ] && echo "$process_name" | grep -q "isgctwing"; then
            echo "$port_pid"
            return 0
        fi
    fi
...
if [ -e "$CONTROL_FILE" ]; then
    ...
    isgctwing_PID=$(get_isgctwing_pid || true)
    if [ -n "$isgctwing_PID" ]; then
        kill "$isgctwing_PID" 2>/dev/null || true
        log "已发送 TERM 信号到 PID $isgctwing_PID"
    else
        log "未找到 isgctwing 进程"
    fi
fi
...
while (( TRIES < MAX_TRIES )); do
    ...
    if [ $TRIES -gt $((MAX_TRIES / 2)) ]; then
        isgctwing_PID=$(get_isgctwing_pid || true)
        if [ -n "$isgctwing_PID" ]; then
            log "发送 KILL 信号强制停止 PID $isgctwing_PID"
            kill -9 "$isgctwing_PID" 2>/dev/null || true
        fi
    fi
