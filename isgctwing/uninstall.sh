...
if [ -f "$SERVICE_DIR/stop.sh" ]; then
    ...
    PID=$(get_isgctwing_pid || echo "")
    if [ -n "$PID" ]; then
        log "尝试杀死 isgctwing 进程 $PID"
...
if get_isgctwing_pid > /dev/null 2>&1; then
    log "警告: isgctwing 进程仍在运行"
...
if [ "$UNINSTALL_STATUS" = "complete" ]; then
    log "isgctwing 完全卸载成功"
    mqtt_report "isg/install/$SERVICE_ID/status" "{\"status\":\"uninstalled\",\"message\":\"isgctwing completely removed\",\"timestamp\":$(date +%s)}"
else
    log "isgctwing 部分卸载，可能需要手动清理"
    mqtt_report "isg/install/$SERVICE_ID/status" "{\"status\":\"uninstalled\",\"message\":\"isgctwing partially removed, manual cleanup may be required\",\"timestamp\":$(date +%s)}"
fi
