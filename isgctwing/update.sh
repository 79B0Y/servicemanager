...
get_version() {
    if dpkg -s isgctwing >/dev/null 2>&1; then
        dpkg -s isgctwing | grep '^Version:' | awk '{print $2}' || echo "unknown"
...
log "starting isgctwing upgrade from $CURRENT_VERSION to $TARGET_VERSION"
...
log "stopping isgctwing service"
...
while [ $WAITED -lt $MAX_WAIT ]; do
    if lsof -i :$PORT >/dev/null 2>&1; then
        ...
        log "isgctwing is running (port $PORT detected)"
