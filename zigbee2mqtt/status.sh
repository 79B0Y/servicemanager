...
check_install_status() {
    test -d "$Z2M_INSTALL_DIR" && test -f "$Z2M_PACKAGE_FILE" && echo "true" || echo "false"
}

get_z2m_version() {
    if [[ -f "$Z2M_PACKAGE_FILE" ]]; then
        # 直接从文件系统读取版本信息，避免 proot 调用
        grep '"version"' "$Z2M_PACKAGE_FILE" 2>/dev/null | head -n1 | cut -d'"' -f4 || echo "unknown"
    else
        echo "unknown"
    fi
}
