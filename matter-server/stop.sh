...
get_matter_pid() {
    ...
    if [ -n "$port_pid" ] && [ "$port_pid" != "-" ]; then
        # 验证是否为 matter-server 相关进程（检查命令行、工作目录或可执行文件）
        ...
        if netstat -tnlp 2>/dev/null | grep ":$MATTER_PORT " | grep -q "$port_pid"; then
            echo "$port_pid"
            return 0
        fi
    fi
...
if [ -e "$CONTROL_FILE" ]; then
    ...
    if [ -n "$MATTER_PID" ]; then
        ...
        log "已发送 TERM 信号到 PID $MATTER_PID"
    else
        log "未找到 Matter Server 进程"
    fi
fi
...
while (( TRIES < MAX_TRIES )); do
    ...
    if [ $TRIES -gt $((MAX_TRIES / 2)) ]; then
        MATTER_PID=$(get_matter_pid || true)
        if [ -n "$MATTER_PID" ]; then
            log "发送 KILL 信号强制停止 PID $MATTER_PID"
