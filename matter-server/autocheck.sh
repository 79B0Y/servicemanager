...
check_install_fast() {
    ...
    if [[ -d "$MATTER_INSTALL_DIR" && -d "$MATTER_VENV_DIR" ]]; then
        echo "success"
    else
...
get_improved_backup_status() {
    ...
    if [[ -d "$BACKUP_DIR" ]]; then
        backup_count=$(ls -1 "$BACKUP_DIR"/matter-server_backup_*.tar.gz 2>/dev/null | wc -l)
    fi
...
get_improved_restore_status() {
    ...
    if [[ -f "$MATTER_CONFIG_FILE" || -f "$MATTER_STORAGE_FILE" ]]; then
        echo "success"
    else
        echo "never"
    fi
...
FINAL_MESSAGE="$FINAL_MESSAGE\"port\":\"$MATTER_PORT\","
