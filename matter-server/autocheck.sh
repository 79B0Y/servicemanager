...
# 版本: v1.0.0
# 优化: 参照 zigbee2mqtt autocheck.sh，不依赖外部文件，快速检查
# =============================================================================
...
MATTER_ENV_DIR="$PROOT_ROOTFS/opt/matter-server/venv"
MATTER_CONFIG_FILE="$PROOT_ROOTFS/opt/matter-server/config.yaml"
...
generate_status_message() {
    ...
    case "$run_status" in
        "running")
            local matter_pid=$(get_matter_pid 2>/dev/null || echo "")
            if [[ -n "$matter_pid" ]]; then
                local uptime_seconds=$(ps -o etimes= -p "$matter_pid" 2>/dev/null | xargs || echo 0)
...
get_config_info_fast() {
    ...
    local mqtt_broker=$(grep -A 10 '^mqtt:' "$MATTER_CONFIG_FILE" 2>/dev/null | grep 'broker:' | awk '{print $2}' | tr -d '"' || echo "")
    local mqtt_username=$(grep -A 10 '^mqtt:' "$MATTER_CONFIG_FILE" 2>/dev/null | grep 'username:' | awk '{print $2}' | tr -d '"' || echo "")
    local listen_ip=$(grep -A 10 '^matter:' "$MATTER_CONFIG_FILE" 2>/dev/null | grep 'listen_ip:' | awk '{print $2}' | tr -d '"' || echo "0.0.0.0")
    local port=$(grep -A 10 '^matter:' "$MATTER_CONFIG_FILE" 2>/dev/null | grep 'port:' | awk '{print $2}' | tr -d '"' || echo "$MATTER_PORT")
    local ssl_enabled=$(grep -A 10 '^matter:' "$MATTER_CONFIG_FILE" 2>/dev/null | grep -A 5 'ssl:' | grep -E '(certfile|keyfile):' | wc -l || echo "0")
    local log_level=$(grep -A 10 '^logging:' "$MATTER_CONFIG_FILE" 2>/dev/null | grep 'level:' | awk '{print $2}' | tr -d '"' || echo "INFO")
    
    # 判断SSL是否启用
    if [[ "$ssl_enabled" -ge 1 ]]; then
        ssl_enabled="true"
    else
        ssl_enabled="false"
    fi
...
# 获取版本信息
# -----------------------------------------------------------------------------
MATTER_VERSION=$(get_current_version_fast)
LATEST_MATTER_VERSION=$(get_latest_version)
...
if [[ -f "$DISABLED_FLAG" ]]; then
    ...
    mqtt_report "isg/autocheck/$SERVICE_ID/status" "{\"status\":\"disabled\",\"run\":\"disabled\",\"config\":$CONFIG_INFO,\"install\":\"$INSTALL_STATUS\",\"update\":\"$UPDATE_STATUS\",\"current_version\":\"$MATTER_VERSION\",\"latest_version\":\"$LATEST_MATTER_VERSION\",\"update_info\":\"$UPDATE_INFO\",\"message\":\"service is disabled\",\"timestamp\":$NOW}"
...
if [[ "$RUN_STATUS" = "stopped" ]]; then
    log "matter-server not running, attempting to start"
...
if [[ -n "$MATTER_PID" ]]; then
    MATTER_UPTIME=$(ps -o etimes= -p "$MATTER_PID" 2>/dev/null | head -n1 | awk '{print $1}' || echo 0)
    # 确保是数字，移除任何非数字字符
    MATTER_UPTIME=$(echo "$MATTER_UPTIME" | tr -d '\n\r\t ' | grep -o '^[0-9]*' || echo 0)
    MATTER_UPTIME=${MATTER_UPTIME:-0}
else
    MATTER_UPTIME=0
fi
...
if [[ "$LAST_CHECK" -gt 0 && "$MATTER_UPTIME" -lt $((NOW - LAST_CHECK)) ]]; then
    RESTART_DETECTED=true
    log "检测到服务重启：运行时间 ${MATTER_UPTIME}s < 检查间隔 $((NOW - LAST_CHECK))s"
fi
...
if [[ -n "$MATTER_PID" ]]; then
    ...
    PS_OUTPUT=$(ps -o pid,pcpu,pmem -p "$MATTER_PID" 2>/dev/null | tail -n1)
    if [[ -n "$PS_OUTPUT" ]]; then
        CPU=$(echo "$PS_OUTPUT" | awk '{print $2}' | head -n1)
        MEM=$(echo "$PS_OUTPUT" | awk '{print $3}' | head -n1)
    else
        # 备用方法：使用 top（较慢）
        CPU=$(top -b -n 1 -p "$MATTER_PID" 2>/dev/null | awk '/'"$MATTER_PID"'/ {print $9}' | head -n1)
        MEM=$(top -b -n 1 -p "$MATTER_PID" 2>/dev/null | awk '/'"$MATTER_PID"'/ {print $10}' | head -n1)
    fi
    # 确保是数字
    CPU=${CPU:-0.0}
    MEM=${MEM:-0.0}
...
log "matter_version: $MATTER_VERSION"
log "latest_matter_version: $LATEST_MATTER_VERSION"
...
mqtt_report "isg/autocheck/$SERVICE_ID/version" "{\"script_version\":\"$SCRIPT_VERSION\",\"latest_script_version\":\"$LATEST_SCRIPT_VERSION\",\"matter_version\":\"$MATTER_VERSION\",\"latest_matter_version\":\"$LATEST_MATTER_VERSION\",\"upgrade_dependencies\":$UPGRADE_DEPS}"
...
if [[ -n "$MATTER_PID" ]]; then
    ...
    if nc -z 127.0.0.1 "$MATTER_PORT" >/dev/null 2>&1; then
...
FINAL_MESSAGE="$FINAL_MESSAGE\"current_version\":\"$MATTER_VERSION\","
FINAL_MESSAGE="$FINAL_MESSAGE\"latest_version\":\"$LATEST_MATTER_VERSION\","
FINAL_MESSAGE="$FINAL_MESSAGE\"update_info\":\"$UPDATE_INFO\","
FINAL_MESSAGE="$FINAL_MESSAGE\"message\":\"$STATUS_MESSAGE\","
FINAL_MESSAGE="$FINAL_MESSAGE\"service_state\":\"$SERVICE_STATE\","
