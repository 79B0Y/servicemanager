...
# 检查 python-matter-server 安装
log_run "Checking python-matter-server installation..."
if ! proot-distro login "$PROOT_DISTRO" -- bash -c "
    source $MATTER_ENV_DIR/bin/activate
    python -c 'import matter_server; print(\"Matter Server version:\", matter_server.__version__)'
" >> "$LOG_FILE" 2>&1; then
    log_run "ERROR: python-matter-server not properly installed"
    exit 1
fi

log_run "All prerequisites checked, starting Matter Server..."

# 启动 Matter Server - 使用 exec 确保进程替换
exec proot-distro login "$PROOT_DISTRO" -- bash -c "
    cd $MATTER_INSTALL_DIR
    source $MATTER_ENV_DIR/bin/activate
    
    # 输出版本信息
    echo \"[INFO] Starting python-matter-server...\"
    echo \"[INFO] Working directory: \$(pwd)\"
    echo \"[INFO] Python version: \$(python --version)\"
    echo \"[INFO] Matter server version: \$(python -c 'import matter_server; print(matter_server.__version__)' 2>/dev/null || echo 'unknown')\"
    
    # 启动 python-matter-server
    exec python -m matter_server --config $MATTER_CONFIG_FILE
" 2>&1

log_run "Matter Server process ended unexpectedly"
EOF
