...
# Matter Server 更新脚本
# 版本: v1.0.0
# 功能: 升级 python-matter-server 到最新版本
# =============================================================================
...
log "开始更新 Matter Server，当前版本: $CURRENT_VERSION"
...
# 执行升级（进入 proot 容器）
# -----------------------------------------------------------------------------
log "更新 python-matter-server 到版本 $TARGET_VERSION"
mqtt_report "isg/update/$SERVICE_ID/status" "{\"status\":\"updating\",\"current_version\":\"$CURRENT_VERSION\",\"message\":\"updating python-matter-server package\",\"timestamp\":$(date +%s)}"

if ! proot-distro login "$PROOT_DISTRO" -- bash -c "
    source $MATTER_VENV_DIR/bin/activate
    pip install --upgrade python-matter-server==$TARGET_VERSION
"; then
    log "python-matter-server 更新失败"
