...
MATTER_INSTALL_PATH="$PROOT_ROOTFS/opt/matter-server"
MATTER_VENV_PATH="$PROOT_ROOTFS/opt/matter-server/venv"
...
check_install_status() {
    test -d "$MATTER_INSTALL_PATH" && test -d "$MATTER_VENV_PATH" && echo "true" || echo "false"
}

get_matter_version() {
    # 方法1: 尝试从运行中的进程获取版本
    if [[ -n "$(get_service_pid 2>/dev/null)" ]]; then
        local runtime_version=$(proot-distro login "$PROOT_DISTRO" -- bash -c "source /opt/matter-server/venv/bin/activate && matter-server --version" 2>/dev/null | head -n1)
    ...
    # 方法2: 尝试从虚拟环境读取版本
    if [[ -d "$MATTER_VENV_PATH" ]]; then
        local venv_version=$(proot-distro login "$PROOT_DISTRO" -- bash -c "source /opt/matter-server/venv/bin/activate && pip show python-matter-server | grep ^Version: | awk '{print \$2}'" 2>/dev/null)
        if [[ -n "$venv_version" && "$venv_version" != "unknown" ]]; then
            echo "$venv_version"
            return
        fi
    fi
    ...
    local found_version=$(proot-distro login "$PROOT_DISTRO" -- bash -c "
        ...
        if [ -f '/opt/matter-server/venv/bin/activate' ]; then
            ...
            pip show python-matter-server 2>/dev/null | grep '^Version:' | awk '{print \$2}' 2>/dev/null
        fi
    " 2>/dev/null)
