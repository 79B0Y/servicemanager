...
# isg-guardian 备份脚本
# 版本: v1.0.0
# 功能: 备份 isg-guardian 数据和配置
# 修复: 按基础要求备份 /root/isg-guardian/ 目录
# =============================================================================
...
# 按基础要求使用 /root/isg-guardian/ 目录
GUARDIAN_DATA_DIR="/root/isg-guardian"
GUARDIAN_VENV_DIR="$GUARDIAN_DATA_DIR/venv"
...
TS=$(date +%Y%m%d-%H%M%S)
DST="$BACKUP_DIR/isg-guardian_backup_${TS}.tar.gz"
START_TIME=$(date +%s)

log "开始 isg-guardian 备份流程"
...
# 使用 Termux 专用的临时目录
TEMP_BACKUP_DIR="$TERMUX_TMP_DIR/isg-guardian_backup_$"
mkdir -p "$TEMP_BACKUP_DIR"

# 创建备份清单文件
BACKUP_MANIFEST="$TEMP_BACKUP_DIR/backup_manifest.txt"
echo "# isg-guardian Backup Manifest" > "$BACKUP_MANIFEST"
echo "# Created: $(date)" >> "$BACKUP_MANIFEST"
echo "# Service: isg-guardian" >> "$BACKUP_MANIFEST"
echo "# Data Source: $GUARDIAN_DATA_DIR" >> "$BACKUP_MANIFEST"
echo "" >> "$BACKUP_MANIFEST"

# 备份 /root/isg-guardian/ 目录下所有数据
if proot-distro login "$PROOT_DISTRO" -- test -d "$GUARDIAN_DATA_DIR"; then
    log "备份 isg-guardian 数据目录: $GUARDIAN_DATA_DIR"
    
    # 从容器内复制整个 isg-guardian 目录
    proot-distro login "$PROOT_DISTRO" -- bash -c "
        if [ -d '$GUARDIAN_DATA_DIR' ]; then
            cd '$(dirname $GUARDIAN_DATA_DIR)'
            tar -czf '$TEMP_BACKUP_DIR/isg-guardian-data.tar.gz' '$(basename $GUARDIAN_DATA_DIR)'
            echo 'isg-guardian-data.tar.gz' >> '$BACKUP_MANIFEST'
        fi
    "
    
    # 统计数据目录大小
    DATA_SIZE=$(proot-distro login "$PROOT_DISTRO" -- bash -c "du -sh '$GUARDIAN_DATA_DIR' 2>/dev/null | awk '{print \$1}'" || echo "unknown")
    ...
    proot-distro login "$PROOT_DISTRO" -- bash -c "
        ...
        if [ -d '$GUARDIAN_DATA_DIR' ]; then
            find '$GUARDIAN_DATA_DIR' -type f 2>/dev/null | head -20 >> '$BACKUP_MANIFEST' || true
            file_count=\$(find '$GUARDIAN_DATA_DIR' -type f 2>/dev/null | wc -l)
            echo \"# Total files: \$file_count\" >> '$BACKUP_MANIFEST'
        fi
    "
else
    log "警告: isg-guardian 数据目录不存在 $GUARDIAN_DATA_DIR"
    echo "# Warning: Data directory not found: $GUARDIAN_DATA_DIR" >> "$BACKUP_MANIFEST"
fi
...
if [ -n "$GUARDIAN_PID" ]; then
    echo "PID: $GUARDIAN_PID" >> "$PROCESS_INFO"
    echo "Command: $(proot-distro login "$PROOT_DISTRO" -- bash -c "ps -p $GUARDIAN_PID -o args=" 2>/dev/null || echo 'N/A')" >> "$PROCESS_INFO"
    echo "Runtime: $(proot-distro login "$PROOT_DISTRO" -- bash -c "ps -p $GUARDIAN_PID -o etime=" 2>/dev/null || echo 'N/A')" >> "$PROCESS_INFO"
fi
echo "process_info.txt" >> "$BACKUP_MANIFEST"

# 收集 isg-guardian 版本信息
VERSION_INFO="$TEMP_BACKUP_DIR/version_info.txt"
echo "# isg-guardian Version Information" > "$VERSION_INFO"
echo "# Date: $(date)" >> "$VERSION_INFO"
CURRENT_VERSION=$(get_current_version)
echo "isg-guardian Version: $CURRENT_VERSION" >> "$VERSION_INFO"
...
OLD_BACKUPS=$(ls -1t "$BACKUP_DIR"/isg-guardian_backup_*.tar.gz 2>/dev/null | tail -n +$((KEEP_BACKUPS + 1)) || true)
...
# 显示当前备份文件列表
CURRENT_BACKUPS=$(ls -1t "$BACKUP_DIR"/isg-guardian_backup_*.tar.gz 2>/dev/null | wc -l || echo 0)
...
# -----------------------------------------------------------------------------
log "备份内容摘要:"
log "  - 数据目录: $(proot-distro login "$PROOT_DISTRO" -- test -d "$GUARDIAN_DATA_DIR" && echo "已备份 ($GUARDIAN_DATA_DIR)" || echo "不存在")"
log "  - 运行状态: 已备份"
log "  - 进程信息: 已备份"
