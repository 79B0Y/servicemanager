...
log "开始更新 Z-Wave JS UI，当前版本: $CURRENT_VERSION"
...
# 执行升级（进入 proot 容器）
# -----------------------------------------------------------------------------
log "更新 Z-Wave JS UI 到版本 $TARGET_VERSION"
mqtt_report "isg/update/$SERVICE_ID/status" "{\"status\":\"updating\",\"current_version\":\"$CURRENT_VERSION\",\"message\":\"updating zwave-js-ui package\",\"timestamp\":$(date +%s)}"
...
if ! proot-distro login "$PROOT_DISTRO" -- bash -c "
    ...
    echo \"更新 zwave-js-ui 到版本: $TARGET_VERSION\"
    
    pnpm update -g zwave-js-ui@$TARGET_VERSION
"; then
    log "Z-Wave JS UI 更新失败"
    mqtt_report "isg/update/$SERVICE_ID/status" "{\"status\":\"failed\",\"message\":\"zwave-js-ui package update failed\",\"current_version\":\"$CURRENT_VERSION\",\"timestamp\":$(date +%s)}"
