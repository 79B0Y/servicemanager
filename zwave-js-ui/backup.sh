...
log "starting zwave-js-ui backup process"
...
if proot-distro login "$PROOT_DISTRO" -- bash -c "tar -czf \"$DST\" -C \"$(dirname $ZWAVE_STORE_DIR)\" \"$(basename $ZWAVE_STORE_DIR)\""; then
    END_TIME=$(date +%s)
