...
# isghotel 停止脚本
# 版本: v1.0.0
# 功能: 通过 isgservicemonitor 停止 isghotel 服务
# =============================================================================
...
if [ -e "$CONTROL_FILE" ]; then
    ...
    isghotel_PID=$(get_isghotel_pid || true)
    if [ -n "$isghotel_PID" ]; then
        kill "$isghotel_PID" 2>/dev/null || true
        log "已发送 TERM 信号到 PID $isghotel_PID"
    else
        log "未找到 isghotel 进程"
    fi
fi
...
while (( TRIES < MAX_TRIES )); do
    ...
    if [ $TRIES -gt $((MAX_TRIES / 2)) ]; then
        isghotel_PID=$(get_isghotel_pid || true)
        if [ -n "$isghotel_PID" ]; then
            log "发送 KILL 信号强制停止 PID $isghotel_PID"
            kill -9 "$isghotel_PID" 2>/dev/null || true
        fi
    fi
