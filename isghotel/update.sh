...
log "starting isghotel upgrade from $CURRENT_VERSION to $TARGET_VERSION"
...
while [ $WAITED -lt $MAX_WAIT ]; do
    if lsof -i :$PORT >/dev/null 2>&1; then
        ...
        log "isghotel is running (port $PORT detected)"
