...
# isghotel 启动脚本
# 版本: v1.0.0
# 功能: 通过 isgservicemonitor 启动 isghotel 服务
# =============================================================================
...
get_isghotel_pid() {
    local port_pid=$(netstat -tnlp 2>/dev/null | grep ":$isghotel_PORT " | awk '{print $7}' | cut -d'/' -f1 | head -n1)
    
    if [ -n "$port_pid" ] && [ "$port_pid" != "-" ]; then
        local process_name=$(ps -p "$port_pid" -o comm= 2>/dev/null)
        if [ -n "$process_name" ] && echo "$process_name" | grep -q "isghotel"; then
            echo "$port_pid"
            return 0
        fi
    fi
...
while (( TRIES < MAX_TRIES )); do
    if get_isghotel_pid > /dev/null 2>&1; then
        ...
        # 验证端口监听
        if netstat -tulnp 2>/dev/null | grep ":$isghotel_PORT " > /dev/null; then
            log "isghotel 服务启动成功"
            ...
            # 验证网络监听状态
            LISTEN_STATUS=$(netstat -tulnp 2>/dev/null | grep ":$isghotel_PORT " | head -n1)
            log "服务正在监听端口 $isghotel_PORT"
            log "监听状态: $LISTEN_STATUS"
            
            exit 0
        else
            log "isghotel 进程启动但端口未监听，继续等待..."
        fi
    fi
    sleep 5
    TRIES=$((TRIES+1))
