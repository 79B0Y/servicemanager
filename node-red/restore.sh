...
EOF
    ...
    log "default node-red configuration generated successfully"
}
...
# 执行恢复
# -----------------------------------------------------------------------------
if proot-distro login "$PROOT_DISTRO" -- bash -c "rm -rf \"$NR_DATA_DIR\" && mkdir -p \"$NR_DATA_DIR\" && tar -xzf \"$FINAL_RESTORE_FILE\" -C \"$(dirname $NR_DATA_DIR)\""; then
