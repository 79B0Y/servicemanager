...
log "starting node-red backup process"
...
if proot-distro login "$PROOT_DISTRO" -- bash -c "tar -czf \"$DST\" -C \"$(dirname $NODE_RED_DATA_DIR)\" \"$(basename $NODE_RED_DATA_DIR)\""; then
    END_TIME=$(date +%s)
