...
NR_DATA_DIR="${NR_DATA_DIR:-/root/.node-red}"
...
NR_VERSION_TEMP="$TERMUX_TMP_DIR/nr_version.txt"
...
# -----------------------------------------------------------------------------
get_install_status() {
    if proot-distro login "$PROOT_DISTRO" -- test -d "$NR_INSTALL_DIR"; then
        if proot-distro login "$PROOT_DISTRO" -- test -f "$NR_PACKAGE_FILE"; then
            echo "success"
        else
            echo "failed"
        fi
...
# -----------------------------------------------------------------------------
get_config_info() {
    if ! proot-distro login "$PROOT_DISTRO" -- test -f "$NR_SETTINGS_FILE"; then
...
get_improved_restore_status() {
    ...
    if [ -f "$LOG_FILE_RESTORE" ] && [ -s "$LOG_FILE_RESTORE" ]; then
        ...
        if echo "$recent_log" | grep -q "restore.*complete\|data restored.*successfully"; then
            echo "success"
