...
# Node-RED 停止脚本
# 版本: v1.1.0
# 功能: 通过 isgservicemonitor 停止 Node-RED 服务
# =============================================================================
...
if [ -e "$CONTROL_FILE" ]; then
    ...
    NODE_RED_PID=$(get_node_red_pid || true)
    if [ -n "$NODE_RED_PID" ]; then
        kill "$NODE_RED_PID" 2>/dev/null || true
        log "sent TERM signal to PID $NODE_RED_PID"
    else
        log "no node-red process found"
    fi
fi
