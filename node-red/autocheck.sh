...
NR_INSTALL_DIR="$PROOT_ROOTFS/opt/node-red"
NR_PACKAGE_FILE="$PROOT_ROOTFS/opt/node-red/package.json"
NR_DATA_DIR="$PROOT_ROOTFS/root/.node-red"
NR_FLOWS_FILE="$PROOT_ROOTFS/root/.node-red/flows.json"
NR_SETTINGS_FILE="$PROOT_ROOTFS/root/.node-red/settings.js"
...
get_current_version_fast() {
    ...
    # 方法1: 直接从文件系统读取版本信息
    if [[ -f "$NR_PACKAGE_FILE" ]]; then
        local version_output=$(grep '"version"' "$NR_PACKAGE_FILE" 2>/dev/null | head -n1 | sed -E 's/.*"version": *"([^"]+)".*/\1/' || echo "unknown")
    ...
    # 方法2: 备用 - 通过 proot 调用获取
    local proot_version=$(proot-distro login "$PROOT_DISTRO" -- bash -c "cd /opt/node-red && cat package.json | grep '\"node-red\"' | grep -v 'start' | sed -E 's/.*\"node-red\": *\"([^\"]+)\".*/\1/'" 2>/dev/null || echo "unknown")
...
check_install_fast() {
    ...
    if [[ -d "$NR_INSTALL_DIR" && -f "$NR_PACKAGE_FILE" ]]; then
        echo "success"
    else
...
get_improved_backup_status() {
    ...
    if [[ -d "$BACKUP_DIR" ]]; then
        backup_count=$(ls -1 "$BACKUP_DIR"/node-red_backup_*.tar.gz 2>/dev/null | wc -l)
    fi
...
generate_status_message() {
    ...
    case "$run_status" in
        "running")
            local nr_pid=$(get_nr_pid 2>/dev/null || echo "")
            if [[ -n "$nr_pid" ]]; then
                local uptime_seconds=$(ps -o etimes= -p "$nr_pid" 2>/dev/null | xargs || echo 0)
...
get_config_info_fast() {
    ...
    local port=$(grep 'uiPort:' "$NR_SETTINGS_FILE" 2>/dev/null | sed -E 's/.*uiPort: *([0-9]+).*/\1/' || echo "1880")
    local admin_auth=$(grep -A5 'adminAuth:' "$NR_SETTINGS_FILE" 2>/dev/null | grep 'type:' | sed -E 's/.*type: *"([^"]*)".*/\1/' || echo "none")
    local https_count=$(grep 'https:' "$NR_SETTINGS_FILE" 2>/dev/null | grep -v '//' | wc -l)
    local theme_count=$(grep 'editorTheme:' "$NR_SETTINGS_FILE" 2>/dev/null | grep -v '//' | wc -l)
    
    local https_enabled="false"
    local theme_enabled="false"
    
    [[ $https_count -gt 0 ]] && https_enabled="true"
    [[ $theme_count -gt 0 ]] && theme_enabled="true"
...
# 获取版本信息
# -----------------------------------------------------------------------------
NR_VERSION=$(get_current_version_fast)
...
if [[ -n "$NR_PID" ]]; then
    ...
    PS_OUTPUT=$(ps -o pid,pcpu,pmem -p "$NR_PID" 2>/dev/null | tail -n1)
    if [[ -n "$PS_OUTPUT" ]]; then
...
if [[ -n "$NR_PID" ]]; then
    # 快速检查：直接使用 nc 检查端口
    if nc -z 127.0.0.1 "$NR_PORT" >/dev/null 2>&1; then
        HTTP_STATUS="online"
    else
