...
# 获取版本信息
# -----------------------------------------------------------------------------
NODE_RED_VERSION=$(get_current_version)
LATEST_NODE_RED_VERSION=$(get_latest_version)
...
# 检查服务重启情况
# -----------------------------------------------------------------------------
NODE_RED_PID=$(get_node_red_pid || echo "")
if [ -n "$NODE_RED_PID" ]; then
    NODE_RED_UPTIME=$(ps -o etimes= -p "$NODE_RED_PID" 2>/dev/null | head -n1 | awk '{print $1}' || echo 0)
    # 确保是数字，移除任何非数字字符
    NODE_RED_UPTIME=$(echo "$NODE_RED_UPTIME" | tr -d '\n\r\t ' | grep -o '^[0-9]*' || echo 0)
    NODE_RED_UPTIME=${NODE_RED_UPTIME:-0}
else
    NODE_RED_UPTIME=0
fi
...
if [ "$LAST_CHECK" -gt 0 ] && [ "$NODE_RED_UPTIME" -lt $((NOW - LAST_CHECK)) ]; then
    RESULT_STATUS="problem"
fi
...
# -----------------------------------------------------------------------------
if [ -n "$NODE_RED_PID" ]; then
    CPU=$(top -b -n 1 -p "$NODE_RED_PID" 2>/dev/null | awk '/'"$NODE_RED_PID"'/ {print $9}' | head -n1)
    MEM=$(top -b -n 1 -p "$NODE_RED_PID" 2>/dev/null | awk '/'"$NODE_RED_PID"'/ {print $10}' | head -n1)
    # 确保是数字
    CPU=${CPU:-0.0}
    MEM=${MEM:-0.0}
else
...
log "node_red_version: $NODE_RED_VERSION"
log "latest_node_red_version: $LATEST_NODE_RED_VERSION"
...
mqtt_report "isg/autocheck/$SERVICE_ID/version" "{\"script_version\":\"$SCRIPT_VERSION\",\"latest_script_version\":\"$LATEST_SCRIPT_VERSION\",\"node_red_version\":\"$NODE_RED_VERSION\",\"latest_node_red_version\":\"$LATEST_NODE_RED_VERSION\",\"upgrade_dependencies\":$UPGRADE_DEPS}"
...
FINAL_MESSAGE="$FINAL_MESSAGE\"current_version\":\"$NODE_RED_VERSION\","
FINAL_MESSAGE="$FINAL_MESSAGE\"latest_version\":\"$LATEST_NODE_RED_VERSION\","
FINAL_MESSAGE="$FINAL_MESSAGE\"update_info\":\"$UPDATE_INFO\","
FINAL_MESSAGE="$FINAL_MESSAGE\"message\":\"$STATUS_MESSAGE\","
FINAL_MESSAGE="$FINAL_MESSAGE\"http_state\":\"$HTTP_STATE\","
