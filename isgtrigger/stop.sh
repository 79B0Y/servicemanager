...
# isgtrigger 停止脚本
# 版本: v1.0.0
# 功能: 通过 isgservicemonitor 停止 isgtrigger 服务
# =============================================================================
...
get_isgtrigger_pid() {
    ...
    if [ -n "$port_pid" ] && [ "$port_pid" != "-" ]; then
        ...
        if [ -n "$process_name" ] && echo "$process_name" | grep -q "isgtrigger"; then
            echo "$port_pid"
            return 0
        fi
    fi
...
if [ -e "$CONTROL_FILE" ]; then
    ...
    ISGTRIGGER_PID=$(get_isgtrigger_pid || true)
    if [ -n "$ISGTRIGGER_PID" ]; then
        kill "$ISGTRIGGER_PID" 2>/dev/null || true
        log "已发送 TERM 信号到 PID $ISGTRIGGER_PID"
    else
        log "未找到 isgtrigger 进程"
    fi
fi
...
while (( TRIES < MAX_TRIES )); do
    ...
    if [ $TRIES -gt $((MAX_TRIES / 2)) ]; then
        ISGTRIGGER_PID=$(get_isgtrigger_pid || true)
        if [ -n "$ISGTRIGGER_PID" ]; then
            log "发送 KILL 信号强制停止 PID $ISGTRIGGER_PID"
            kill -9 "$ISGTRIGGER_PID" 2>/dev/null || true
        fi
    fi
