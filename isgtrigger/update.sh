...
# =============================================================================
# isgtrigger 升级脚本 - 独立版本
# 版本: v1.0.0
# 功能: 升级 isgtrigger 到指定版本
...
get_version() {
    if dpkg -s isgtrigger >/dev/null 2>&1; then
        dpkg -s isgtrigger | grep '^Version:' | awk '{print $2}' || echo "unknown"
...
log "starting isgtrigger upgrade from $CURRENT_VERSION to $TARGET_VERSION"
...
log "stopping isgtrigger service"
...
while [ $WAITED -lt $MAX_WAIT ]; do
    if lsof -i :$PORT >/dev/null 2>&1; then
        ...
        log "isgtrigger is running (port $PORT detected)"
