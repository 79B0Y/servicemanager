...
if [ -f "$SERVICE_DIR/stop.sh" ]; then
    ...
    PID=$(get_isgtrigger_pid || echo "")
    if [ -n "$PID" ]; then
        log "尝试杀死 isgtrigger 进程 $PID"
...
if get_isgtrigger_pid > /dev/null 2>&1; then
    log "警告: isgtrigger 进程仍在运行"
    UNINSTALL_STATUS="partial"
else
    log "✅ isgtrigger 进程已停止"
fi
...
if [ "$UNINSTALL_STATUS" = "complete" ]; then
    log "isgtrigger 完全卸载成功"
    mqtt_report "isg/install/$SERVICE_ID/status" "{\"status\":\"uninstalled\",\"message\":\"isgtrigger completely removed\",\"timestamp\":$(date +%s)}"
else
    log "isgtrigger 部分卸载，可能需要手动清理"
    mqtt_report "isg/install/$SERVICE_ID/status" "{\"status\":\"uninstalled\",\"message\":\"isgtrigger partially removed, manual cleanup may be required\",\"timestamp\":$(date +%s)}"
fi
