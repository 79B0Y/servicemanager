...
# =============================================================================
# isgelecstat 升级脚本 - 独立版本
# 版本: v1.0.0
# 功能: 升级 isgelecstat 到指定版本
...
get_version() {
    if dpkg -s isgelecstat >/dev/null 2>&1; then
        dpkg -s isgelecstat | grep '^Version:' | awk '{print $2}' || echo "unknown"
...
log "starting isgelecstat upgrade from $CURRENT_VERSION to $TARGET_VERSION"
...
log "stopping isgelecstat service"
...
while [ $WAITED -lt $MAX_WAIT ]; do
    if lsof -i :$PORT >/dev/null 2>&1; then
        ...
        log "isgelecstat is running (port $PORT detected)"
