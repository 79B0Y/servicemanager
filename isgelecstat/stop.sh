...
# isgelecstat 停止脚本
# 版本: v1.0.0
# 功能: 通过 isgservicemonitor 停止 isgelecstat 服务
# =============================================================================
...
get_isgelecstat_pid() {
    local port_pid=$(netstat -tnlp 2>/dev/null | grep ":$isgelecstat_PORT " | awk '{print $7}' | cut -d'/' -f1 | head -n1)
    
    if [ -n "$port_pid" ] && [ "$port_pid" != "-" ]; then
        local process_name=$(ps -p "$port_pid" -o comm= 2>/dev/null)
        if [ -n "$process_name" ] && echo "$process_name" | grep -q "isgelecstat"; then
            echo "$port_pid"
            return 0
        fi
    fi
...
if [ -e "$CONTROL_FILE" ]; then
    ...
    isgelecstat_PID=$(get_isgelecstat_pid || true)
    if [ -n "$isgelecstat_PID" ]; then
        kill "$isgelecstat_PID" 2>/dev/null || true
        log "已发送 TERM 信号到 PID $isgelecstat_PID"
    else
        log "未找到 isgelecstat 进程"
    fi
fi
...
while (( TRIES < MAX_TRIES )); do
    ...
    if [ $TRIES -gt $((MAX_TRIES / 2)) ]; then
        isgelecstat_PID=$(get_isgelecstat_pid || true)
        if [ -n "$isgelecstat_PID" ]; then
            log "发送 KILL 信号强制停止 PID $isgelecstat_PID"
            kill -9 "$isgelecstat_PID" 2>/dev/null || true
        fi
    fi
