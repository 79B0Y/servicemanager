...
# =============================================================================
# isgspacemanager 启动脚本
# 版本: v1.0.0
# 功能: 通过 isgservicemonitor 启动 isgspacemanager 服务
# =============================================================================
...
while (( TRIES < MAX_TRIES )); do
    if get_isgspacemanager_pid > /dev/null 2>&1; then
        ...
        # 验证端口监听
        if netstat -tulnp 2>/dev/null | grep ":$isgspacemanager_PORT " > /dev/null; then
            log "isgspacemanager 服务启动成功"
            ...
            # 验证网络监听状态
            LISTEN_STATUS=$(netstat -tulnp 2>/dev/null | grep ":$isgspacemanager_PORT " | head -n1)
            log "服务正在监听端口 $isgspacemanager_PORT"
            log "监听状态: $LISTEN_STATUS"
            
            exit 0
        else
            log "isgspacemanager 进程启动但端口未监听，继续等待..."
        fi
    fi
    sleep 5
    TRIES=$((TRIES+1))
