...
# =============================================================================
# isg-credential-services 状态查询脚本 - 优化版本
# 版本: v1.0.0
...
CREDENTIAL_INSTALL_PATH="$PROOT_ROOTFS/root/isg-credential-services"
...
get_service_pid() {
    ...
    if [[ -n "$port_pid" && "$port_pid" != "-" ]]; then
        # 验证是否为 isg-credential-services 相关进程（检查工作目录或命令行）
...
check_install_status() {
    test -d "$CREDENTIAL_INSTALL_PATH" && echo "true" || echo "false"
}

get_credential_version() {
    # 方法1: 尝试从运行中的进程获取版本
    if [[ -n "$(get_service_pid 2>/dev/null)" ]]; then
        local runtime_version=$(proot-distro login "$PROOT_DISTRO" -- bash -c "cd /root/isg-credential-services && bash manage-service.sh version" 2>/dev/null | head -n1)
    ...
    # 方法2: 尝试从安装目录读取版本
    if [[ -d "$CREDENTIAL_INSTALL_PATH" ]]; then
        local version=$(proot-distro login "$PROOT_DISTRO" -- bash -c "cd /root/isg-credential-services && bash manage-service.sh version" 2>/dev/null)
...
if [[ "$IS_JSON_MODE" -eq 0 && "$IS_QUIET_MODE" -eq 0 ]]; then
    ...
    if [[ "$STATUS_MODE" != "1" ]]; then
        ...
        if [[ "$INSTALL_STATUS" == "true" && "$VERSION" == "unknown" ]]; then
            VERSION=$(get_credential_version)
        fi
    fi
