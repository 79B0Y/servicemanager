...
# =============================================================================
# isgaddonmanager 升级脚本 - 独立版本
# 版本: v1.0.0
# 功能: 升级 isgaddonmanager 到指定版本
...
get_version() {
    if dpkg -s isgaddonmanager >/dev/null 2>&1; then
        dpkg -s isgaddonmanager | grep '^Version:' | awk '{print $2}' || echo "unknown"
...
log "starting isgaddonmanager upgrade from $CURRENT_VERSION to $TARGET_VERSION"
...
log "stopping isgaddonmanager service"
...
if ! wget -q "https://eucfg.linklinkiot.com/isg/${SERVICE_ID}-${TARGET_VERSION}_termux_arm.deb" -O "$SERVICE_ID.deb"; then
...
while [ $WAITED -lt $MAX_WAIT ]; do
    if lsof -i :$PORT >/dev/null 2>&1; then
        ...
        log "isgaddonmanager is running (port $PORT detected)"
