...
##  21. 启用MQTT维护功能
### 1. 完整维护功能
bashMQTT_MAINTENANCE_ENABLED=1 bash autocheck.sh
默认执行监控，不做清理和优化

MQTT_MAINTENANCE_ENABLED=1 MQTT_MAINTENANCE_TYPE=full bash autocheck.sh
执行完整维护：监控 + 清理 + 优化
### 2. 仅启用清理功能
bashMQTT_CLEANUP_ENABLED=1 bash autocheck.sh
仅执行normal级别清理

MQTT_CLEANUP_ENABLED=1 MQTT_CLEANUP_LEVEL=aggressive bash autocheck.sh
执行aggressive级别清理
### 3. 仅启用优化功能
bashMQTT_OPTIMIZE_ENABLED=1 bash autocheck.sh
仅执行normal级别优化

MQTT_OPTIMIZE_ENABLED=1 MQTT_OPTIMIZE_LEVEL=memory bash autocheck.sh
执行memory级别优化

📋 环境变量完整列表
配置管理

MQTT_USERNAME - 新的MQTT用户名
MQTT_PASSWORD - 新的MQTT密码

MQTT维护控制

MQTT_MAINTENANCE_ENABLED=1 - 启用完整维护功能
MQTT_MAINTENANCE_TYPE - 维护类型：monitor(默认)|cleanup|optimize|full

清理功能

MQTT_CLEANUP_ENABLED=1 - 仅启用清理功能
MQTT_CLEANUP_LEVEL - 清理级别：normal(默认)|aggressive|emergency

优化功能

MQTT_OPTIMIZE_ENABLED=1 - 仅启用优化功能
MQTT_OPTIMIZE_LEVEL - 优化级别：normal(默认)|performance|memory

🔄 实际使用场景
日常运维（推荐）
bash# 定时任务：每天检查，每周清理
crontab 设置
0 */6 * * * bash /path/to/autocheck.sh  # 每6小时基础检查
0 2 * * 1 MQTT_CLEANUP_ENABLED=1 bash /path/to/autocheck.sh  # 每周一清理
问题排查
bash# 发现性能问题时
MQTT_MAINTENANCE_ENABLED=1 MQTT_MAINTENANCE_TYPE=full bash autocheck.sh
...
一次性维护
bash# 彻底清理和优化
MQTT_CLEANUP_ENABLED=1 MQTT_CLEANUP_LEVEL=emergency MQTT_OPTIMIZE_ENABLED=1 MQTT_OPTIMIZE_LEVEL=performance bash autocheck.sh
✅ 优势
...
现在 autocheck.sh 既保持了原有的简洁性，又提供了强大的可选维护功能！
