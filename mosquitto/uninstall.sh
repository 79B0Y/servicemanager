...
# 验证所有进程已终止
REMAINING_PIDS=$(pgrep -f mosquitto 2>/dev/null || true)
...
# 检查并强制释放1883端口
PIDS_ON_1883=$(netstat -tulnp 2>/dev/null | grep ":1883" | awk '{print $7}' | cut -d'/' -f1 | grep -v '^- || true)
...
# 检查并强制释放9001端口
PIDS_ON_9001=$(netstat -tulnp 2>/dev/null | grep ":9001" | awk '{print $7}' | cut -d'/' -f1 | grep -v '^- || true)
...
# 验证端口释放
if netstat -tulnp 2>/dev/null | grep -E ":1883|:9001" >/dev/null; then
    log_warn "some mosquitto ports still in use after cleanup"
    netstat -tulnp 2>/dev/null | grep -E ":1883|:9001" >> "$LOG_FILE" || true
else
...
find /data/data/com.termux/files/usr -name "*mosquitto*" -type f 2>/dev/null | while read -r file; do
    if [ -f "$file" ]; then
        rm -f "$file"
...
# 验证进程清理
REMAINING_MOSQUITTO_PROCESSES=$(pgrep -f mosquitto 2>/dev/null || true)
...
# 验证端口清理
REMAINING_MOSQUITTO_PORTS=$(netstat -tulnp 2>/dev/null | grep -E ":1883|:9001" || true)
