...
# Home Assistant 停止脚本
# 版本: v1.4.0
# 功能: 通过 isgservicemonitor 停止 Home Assistant 服务
# =============================================================================
...
log "stopping Home Assistant service"
...
if [ -e "$CONTROL_FILE" ]; then
    ...
    HA_PID=$(get_ha_pid || true)
    if [ -n "$HA_PID" ]; then
        kill "$HA_PID" 2>/dev/null || true
        log "sent TERM signal to PID $HA_PID"
    else
        log "no Home Assistant process found"
    fi
fi
