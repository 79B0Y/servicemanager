...
log "starting Home Assistant backup process"
...
if proot-distro login "$PROOT_DISTRO" -- bash -c "tar -czf \"$DST\" -C \"$(dirname $HA_CONFIG_DIR)\" \"$(basename $HA_CONFIG_DIR)\""; then
    END_TIME=$(date +%s)
...
OLD_BACKUPS=$(ls -1t "$BACKUP_DIR"/homeassistant_backup_*.tar.gz 2>/dev/null | tail -n +$((KEEP_BACKUPS + 1)) || true)
...
# 显示当前备份文件列表
CURRENT_BACKUPS=$(ls -1t "$BACKUP_DIR"/homeassistant_backup_*.tar.gz 2>/dev/null | wc -l || echo 0)
