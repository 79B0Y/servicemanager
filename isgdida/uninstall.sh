...
if [ -f "$SERVICE_DIR/stop.sh" ]; then
    ...
    PID=$(get_isgdida_pid || echo "")
    if [ -n "$PID" ]; then
        log "尝试杀死 isgdida 进程 $PID"
...
if get_isgdida_pid > /dev/null 2>&1; then
    log "警告: isgdida 进程仍在运行"
...
if [ "$UNINSTALL_STATUS" = "complete" ]; then
    log "isgdida 完全卸载成功"
    mqtt_report "isg/install/$SERVICE_ID/status" "{\"status\":\"uninstalled\",\"message\":\"isgdida completely removed\",\"timestamp\":$(date +%s)}"
else
    log "isgdida 部分卸载，可能需要手动清理"
    mqtt_report "isg/install/$SERVICE_ID/status" "{\"status\":\"uninstalled\",\"message\":\"isgdida partially removed, manual cleanup may be required\",\"timestamp\":$(date +%s)}"
fi
