...
get_version() {
    if dpkg -s isgdida >/dev/null 2>&1; then
        dpkg -s isgdida | grep '^Version:' | awk '{print $2}' || echo "unknown"
...
log "starting isgdida upgrade from $CURRENT_VERSION to $TARGET_VERSION"
...
while [ $WAITED -lt $MAX_WAIT ]; do
    if lsof -i :$PORT >/dev/null 2>&1; then
        ...
        log "isgdida is running (port $PORT detected)"
