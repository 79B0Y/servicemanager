...
# isgdida 停止脚本
# 版本: v1.0.0
# 功能: 通过 isgservicemonitor 停止 isgdida 服务
# =============================================================================
...
if [ -e "$CONTROL_FILE" ]; then
    ...
    isgdida_PID=$(get_isgdida_pid || true)
    if [ -n "$isgdida_PID" ]; then
        kill "$isgdida_PID" 2>/dev/null || true
        log "已发送 TERM 信号到 PID $isgdida_PID"
    else
        log "未找到 isgdida 进程"
    fi
fi
...
while (( TRIES < MAX_TRIES )); do
    ...
    if [ $TRIES -gt $((MAX_TRIES / 2)) ]; then
        isgdida_PID=$(get_isgdida_pid || true)
        if [ -n "$isgdida_PID" ]; then
            log "发送 KILL 信号强制停止 PID $isgdida_PID"
            kill -9 "$isgdida_PID" 2>/dev/null || true
        fi
    fi
