...
class ADBController:
    ...
    async def _run(self, *args: str, timeout: float = 15.0, _retry: bool = True) -> str:
        ...
        If a device connection error is detected, attempt a one-time
        reconnect via `connect()` and retry the command once.
        """
        ...
        if proc.returncode != 0:
            ...
            if ("device offline" in lower_err) or ("device not found" in lower_err):
                ...
                # Try a one-time reconnect then retry
                if _retry:
                    try:
                        await self.connect()
                    except Exception as e:
                        ...
                        return await self._run(*args, timeout=timeout, _retry=False)
    ...
    async def connect(self) -> str:
        """Ensure ADB server is running and connect to target device.

        Important: Do NOT pass "-s" when running host-level commands like
        "adb start-server" or "adb connect", because the device may not
        be available yet. After a successful connect, use "wait-for-device"
        scoped to the target to ensure the transport is ready.
        """
        ...
        if self.serial:
            ...
            try:
                await self._run("wait-for-device", timeout=12.0)
            except Exception as e:
        ...
        try:
            # Start adb server once to avoid smartsocket races when many commands
            # hit the server concurrently at startup.
            try:
                proc = await asyncio.create_subprocess_exec(
                    "adb", "start-server",
                    stdout=asyncio.subprocess.PIPE,
            ...
            out, err = await asyncio.wait_for(proc2.communicate(), timeout=10.0)
            ...
            # Wait for the target transport to become ready
            try:
                await self._run("wait-for-device", timeout=12.0)
            except Exception as e:
                logger.debug("wait-for-device failed/ignored: %s", e)
