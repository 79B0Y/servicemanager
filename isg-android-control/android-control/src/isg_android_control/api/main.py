...
shots = ScreenshotService(adb, Path.cwd() / settings.device.screenshots_dir, keep=settings.device.screenshot_keep)
cache = Cache()
...
async def apps_options() -> dict:
    ...
    return {"options": visible, "mapping": settings.appmap.apps, "installed": installed_map, "active": active_name}
...
async def ha_refresh() -> dict:
    ...
    # Clear and republish discovery + attributes (strong refresh)
    ...
    active_pkg = info.get("package")
    active_name = next((n for n, p in settings.appmap.apps.items() if p == active_pkg), None)
    # Republish discovery + attributes
    ha.publish_discovery(options, has_battery=settings.device.has_battery, has_cellular=settings.device.has_cellular)
    ha.publish_app_attributes(options, settings.appmap.apps, installed=installed_map, active=active_name)
...
async def ha_update_options(payload: OptionsUpdate) -> dict:
    # Validate options exist in app mapping
    ...
    # Update in-memory and republish HA discovery + attributes
    ...
    if ha:
        ...
        active_pkg = info.get("package")
        active_name = next((n for n, p in settings.appmap.apps.items() if p == active_pkg), None)
        ha.publish_discovery(payload.options, has_battery=settings.device.has_battery, has_cellular=settings.device.has_cellular)
        ha.publish_app_attributes(payload.options, settings.appmap.apps, installed=installed_map, active=active_name)
