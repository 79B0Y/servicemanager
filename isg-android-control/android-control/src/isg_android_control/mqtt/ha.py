...
class HAIntegration:
    ...
    def clear_discovery(self, app_names: list[str]) -> None:
        ...
        for comp, ids in {
            ...
            # sensors
            "sensor": [
                "battery","cpu_usage","memory_used","network_type","foreground_app","battery_temp","battery_health","storage_used","cell_level"
            ],
        }.items():
            for oid in ids:
    ...
    def publish_discovery(self, app_names: list[str], has_battery: bool = False, has_cellular: bool = False, camera: bool = True) -> None:
        ...
        switch_payload = {
            ...
            "value_template": "{{ 'ON' if value_json.screen.on else 'OFF' }}",
        ...
        vol_pct = {
            ...
            "value_template": "{{ (value_json.audio.music.percent | default(0, true) | int(0)) }}",
            "unit_of_measurement": "%",
            "min": 0,
            "max": 100,
            "step": 1,
    ...
    def publish_state(self, state: dict) -> None:
        # Retain last known state so HA number sliders don't show unavailable
        self.client.publish(f"{self.cfg.base_topic}/state", json.dumps(state), retain=True)
