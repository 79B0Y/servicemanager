...
# =============================================================================
# isg-android-control 启动脚本
# 版本: v1.0.0
# 功能: 通过 isgservicemonitor 启动 isg-android-control 服务
# =============================================================================
...
while (( TRIES < MAX_TRIES )); do
    if get_android_control_pid > /dev/null 2>&1; then
        ...
        if bash "$SERVICE_DIR/status.sh" --quiet; then
            log "isg-android-control 服务启动成功"
            ...
            log "isg-android-control 进程启动但状态检查未通过，继续等待..."
        fi
    fi
    sleep 5
    TRIES=$((TRIES+1))
