...
# Z-Wave JS UI 启动脚本
# 版本: v1.0.0
# 功能: 通过 isgservicemonitor 启动 Z-Wave JS UI 服务
# =============================================================================
    ...
    if get_zwave_pid > /dev/null 2>&1; then
        ...
        if timeout 5 nc -z 127.0.0.1 "$ZWAVE_PORT" 2>/dev/null; then
            ...
            if echo "$LISTEN_STATUS" | grep "0.0.0.0:$ZWAVE_PORT" > /dev/null; then
                log "服务正在监听 0.0.0.0:$ZWAVE_PORT (全局访问)"
            else
                log "服务监听状态: $LISTEN_STATUS"
            fi
            
            exit 0
        else
            log "zwave-js-ui 进程启动但端口未监听，继续等待..."
        fi
    fi
    sleep 5
    TRIES=$((TRIES+1))
