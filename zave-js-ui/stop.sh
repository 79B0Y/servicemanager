...
# Z-Wave JS UI 停止脚本
# 版本: v1.0.0
# 功能: 通过 isgservicemonitor 停止 Z-Wave JS UI 服务
# =============================================================================
...
if [ -e "$CONTROL_FILE" ]; then
    ...
    ZWAVE_PID=$(get_zwave_pid || true)
    if [ -n "$ZWAVE_PID" ]; then
        kill "$ZWAVE_PID" 2>/dev/null || true
        log "已发送 TERM 信号到 PID $ZWAVE_PID"
    else
        log "未找到 zwave-js-ui 进程"
    fi
fi
...
while (( TRIES < MAX_TRIES )); do
    ...
    if [ $TRIES -gt $((MAX_TRIES / 2)) ]; then
        ZWAVE_PID=$(get_zwave_pid || true)
        if [ -n "$ZWAVE_PID" ]; then
            log "发送 KILL 信号强制停止 PID $ZWAVE_PID"
