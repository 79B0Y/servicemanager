...
3) 确保pnpm global安装路径在 /root/.pnpm-global/global/5/node_modules/ 下面
    ```bash
...
---
# -----------------------------------------------------------------------------
# ⬆️ 生成启动脚本（路径改为 /sdcard/isgbackup/matter-bridge/）
# -----------------------------------------------------------------------------
```bash
...
```bash
if ! proot-distro login "$PROOT_DISTRO" -- bash -c "
    pnpm remove -g home-assistant-matter-hub
"; then
    log "failed to uninstall matter-bridge"
    mqtt_report "isg/install/$SERVICE_ID/status" "{\"status\":\"failed\",\"message\":\"matter-bridge uninstallation failed\",\"timestamp\":$(date +%s)}"
    record_install_history "FAILED" "unknown"
    exit 1
fi
```
卸载需要写入log，uninstall.log
...
### 9. 升级命令
pnpm add -g home-assistant-matter-hub@latest
